import { Collection, CreateCollectionOptions, CreateIndexesOptions, Db, IndexSpecification, MongoClient, MongoClientOptions } from "mongodb"
...
export class MongoDbIgniter {
    public static async initializeDb(dbDeclaration: DbDeclaration, purge = false): Promise<Db> {
        ...
        const collectionInitializations = dbDeclaration.collections.map(collection => MongoDbIgniter.initializeCollection(db, collection, purge))
    ...
    public static async initializeCollection(
        ...
        const collection =
            db.collection(collectionDeclaration.name) || (await db.createCollection(collectionDeclaration.name, collectionDeclaration.options))
        ...
        if (collectionDeclaration.indexes) {
            ...
            const indexInitializations = collectionDeclaration.indexes.map(index => MongoDbIgniter.initializeIndex(collection, index))
